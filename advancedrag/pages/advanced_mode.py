...
def format_multi_method_response(user_input, multi_responses, csv_answer=None, feedback_msg=""):
    ...
    # Find the method with the highest BERTScore
    best_method = None
    best_bert_score = -1
    ...
    # Store the best response for audio generation
    if best_method:
    ...
    for method in ["hybrid", "semantic", "lexical"]:
        ...
        # Add a visual indicator for the best method
        best_indicator = " (Best BERTScore)" if method == best_method else ""
        ...
        response += f"<h4>{method.capitalize()} Search (BERTScore: {bert_score:.4f} | Rouge-L: {rouge_l_score:.4f}{method_info}){best_indicator}</h4>"
    ...
    try:
        ...
        for manual_name in subfolders:
            ...
            for img_path in manual_images:
                ...
                # search for something like page_123_img
                match = re.search(r'page_(\d+)_img_', img_filename)
                if match:
                    # in this case takes the value 123
                ...
                if index is not None and texts and bm25_model and tokenized_corpus:
                    ...
                    if not images_already_extracted:
                        with st.spinner("Extracting images from PDFs..."):
                            results, total_images, total_pdfs = process_pdf_images_wrapper(folder_path,
    ...
    if not st.session_state.chat_history:
        ...
        for i, (sender, message) in enumerate(st.session_state.chat_history):
            if sender == 'You':
                ...
                if st.button("🔊 Listen to Best Response", key=tts_key):
                    # Check if this is a multi-method response and we have a best response stored
                    if hasattr(st.session_state,
                               'best_response_for_audio') and st.session_state.best_response_for_audio:
                        # Use the best response for audio
                        clean_text = clean_text_for_audio(st.session_state.best_response_for_audio)
                    else:
                        # Fallback to the full message for single method responses
                        ...
                        with st.spinner("Generating response..."):
                            ...
                            if csv_answer:
                                ...
                                log_max_bertscore_to_csv(
                                    ...
                                    bert_score_val,  # max_bert_score
                                    response_answer_score,  # response_answer_bert_score
                                    max_chunk_answer_score,  # max_chunk_answer_bert_score
                                    selected_pdf,  # selected_pdf
                                    llm_model,  # llm_model
                                    search_type,  # search_type
                                    rouge_l_val,  # max_rouge_score
                                    response_answer_rouge_score,  # response_answer_rouge_score
                                    max_chunk_answer_rouge_score,  # max_chunk_answer_rouge_score
                                    st.session_state.get('use_reranker', True),  # use_reranker
                                    st.session_state.get('reranker_model', get_default_reranker_model()),
                                    chunking_method,
                                    st.session_state.get('use_query_optimization'),
                                    embedding_model
